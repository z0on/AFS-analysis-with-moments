...
import sys
infile=sys.argv[1]
params=array([  float(sys.argv[2]),   float(sys.argv[3]),  float(sys.argv[4]),   float(sys.argv[5]),   float(sys.argv[6])])
...
func=g2
upper_bound = [100,100,10,10,0.25]
lower_bound = [1e-3,1e-3, 1e-3,1e-3,1e-10]
...
print "g2",ind, sys.argv[1],' ll: ', ll_model,' p: ', poptg, " t: ",theta

moments.Plotting.plot_1d_comp_multinom(model, data)

plt.savefig("g2_"+ind+"_"+sys.argv[1]+'.pdf')
