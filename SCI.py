...
# islands:
    ...
    fsi.integrate([nu1, nu2], T1, m = np.array([[0, m12i], [m21i, 0]]))
    
    fs2=P*fsi+(1-P)*fs
    return (1-p_misid)*fs2 + p_misid*moments.Numerics.reverse_array(fs2)
 
func=SCI
upper_bound = [100, 100, 100,100, 200,200,200,200,0.9999,0.25]
lower_bound = [1e-3,1e-3, 1e-3,1e-3,0.01,0.01,0.01,0.01,1e-4,1e-5]
...
moments.ModelPlot.plot_model(plot_mod, save_file="SCI_"+ind+".png", pop_labels=pop_ids, nref=theta/(4*mu), gen_time=gtime, gen_time_units="KY", reverse_timeline=True)
...
print "SCI_Res",ind,sys.argv[1],sys.argv[2],sys.argv[3],' ll: ', ll_model,' p: ', poptg, " t: ",theta, 'uncert: ', uncert
                                    
# plotting quad-panel figure wit AFS, model, residuals:
moments.Plotting.plot_2d_comp_multinom(model, data, vmin=1, resid_range=3,
                                    pop_ids =pop_ids)
plt.savefig("SCI"+ind+"_"+sys.argv[1]+"_"+sys.argv[2]+"_"+sys.argv[3]+"_"+sys.argv[4]+"_"+sys.argv[5]+'.pdf')
