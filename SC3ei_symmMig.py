...
# split, three epochs in each pop, symmetric migration in second and third epoch
# genomic islands
# n(para): 15
...
def sc3eism(params , ns):
#    p_misid: proportion of misidentified ancestral states
    nu1_1, nu2_1, nu1_2,nu2_2,nu1_3,nu2_3,T1, T2, T3,m1,m2,m1i,m2i, P, p_misid = params
    sts = moments.LinearSystem_1D.steady_state_1D(ns[0] + ns[1])
    fs = moments.Spectrum(sts)
    fs = moments.Manips.split_1D_to_2D(fs, ns[0], ns[1])
    fs.integrate([nu1_1, nu2_1], T1, m = np.array([[0, 0], [0, 0]]))
    fs.integrate([nu1_2, nu2_2], T2, m = np.array([[0, m2], [m2, 0]]))
    fs.integrate([nu1_3, nu2_3], T3, m = np.array([[0, m3], [m3, 0]]))

    stsi = moments.LinearSystem_1D.steady_state_1D(ns[0] + ns[1])
    fsi = moments.Spectrum(stsi)
    fsi = moments.Manips.split_1D_to_2D(fsi, ns[0], ns[1])
    fsi.integrate([nu1_1, nu2_1], T1, m = np.array([[0, 0], [0, 0]]))
    fsi.integrate([nu1_2, nu2_2], T2, m = np.array([[0, m2i], [m2i, 0]]))
    fsi.integrate([nu1_3, nu2_3], T3, m = np.array([[0, m3i], [m3i, 0]]))
...
moments.ModelPlot.plot_model(plot_mod, save_file="sc3eism_"+ind+".png", pop_labels=pop_ids, nref=theta/(4*mu), gen_time=gtime, gen_time_units="KY", reverse_timeline=True)
...
print "RESULT","sc3eism",ind,len(params),ll_model,sys.argv[1],sys.argv[2],sys.argv[3],poptg,theta,uncert
                                    
# plotting quad-panel figure witt AFS, model, residuals:
moments.Plotting.plot_2d_comp_multinom(model, data, vmin=1, resid_range=3,
                                    pop_ids =pop_ids)
plt.savefig("sc3eism_"+ind+"_"+sys.argv[1]+"_"+sys.argv[2]+"_"+sys.argv[3]+"_"+sys.argv[4]+"_"+sys.argv[5]+'.pdf')
