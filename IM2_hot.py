...
moments.ModelPlot.plot_model(plot_mod, save_file="IM2"+ind+"_"+sys.argv[1]+".png",pop_labels=pop_ids, nref=theta/(4*mu), gen_time=0.005, gen_time_units="KY", reverse_timeline=True)
...
print "IM2",ind,sys.argv[1],sys.argv[2],sys.argv[3],' ll: ', ll_model,' p: ', poptg, " t: ",theta, 'uncert: ', uncert
moments.Plotting.plot_2d_comp_multinom(model, data, vmin=1, resid_range=3,
                                    pop_ids =pop_ids)
                                    
# plotting quad-panel figure wit AFS, model, residuals:
plt.savefig("IM2"+ind+"_"+sys.argv[1]+"_"+sys.argv[2]+"_"+sys.argv[3]+"_"+sys.argv[4]+"_"+sys.argv[5]+'.pdf')
