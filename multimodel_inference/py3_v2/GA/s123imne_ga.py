...
# Fi: factor of migration reduction (1e-5 - 0.999)
    nu0,nu1_1,nu2_1,nu1_2,nu2_2,T0,T1,T2,m12,m21,Fi,P,p_misid = params
    ...
    fs.integrate([nu1_1, nu2_1], T1, m = np.array([[0, m12*nu2_1*Fi], [m21*nu1_1*Fi, 0]]))
    fs.integrate([nu1_2, nu2_2], T2, m = np.array([[0, m12*nu2_2*Fi], [m21*nu1_2*Fi, 0]]))
...
par_labels = ('nu0','nu1_1','nu2_1','nu1_2','nu2_2','T0','T1','T2','m12','m21','F_isl','F_gen','f_misid')
...
moments.ModelPlot.plot_model(plot_mod, save_file="s123imne_"+ind+".png", pop_labels=pop_ids, nref=theta/(4*mu), draw_scale=False, gen_time=gtime, gen_time_units="KY", reverse_timeline=True)

# bootstrapping for SDs of params and theta

# printing parameters and their SDs
print( "RESULT","s123imne",ind,len(par_labels),ll_model,sys.argv[1],sys.argv[2],sys.argv[3],poptg,theta)
                                    
# plotting quad-panel figure witt AFS, model, residuals:
moments.Plotting.plot_2d_comp_multinom(model, data, vmin=0.1, resid_range=3,
                                    pop_ids =pop_ids)
plt.savefig("s123imne_"+ind+"_"+sys.argv[1]+"_"+sys.argv[2]+"_"+sys.argv[3]+"_"+sys.argv[4]+"_"+sys.argv[5]+'.pdf')
