...
# Fs: factor of Ne reduction (0.1 - 0.999)
    ...
    fsis.integrate([nu1_2*Fs, nu2_2*Fs], T2, m = np.array([[0, m12*Fi], [m21*Fi, 0]]))
    ...
    fss.integrate([nu1_2*Fs, nu2_2*Fs], T2, m = np.array([[0, m12], [m21, 0]]))
...
moments.ModelPlot.plot_model(plot_mod, save_file="scSi_"+ind+".png", pop_labels=pop_ids, nref=theta/(4*mu), draw_scale=False, gen_time=gtime, gen_time_units="KY", reverse_timeline=True)

# bootstrapping for SDs of params and theta

# printing parameters and their SDs
print( "RESULT","scSi",ind,len(par_labels),ll_model,sys.argv[1],sys.argv[2],sys.argv[3],poptg,theta)
                                    
# plotting quad-panel figure witt AFS, model, residuals:
moments.Plotting.plot_2d_comp_multinom(model, data, vmin=0.1, resid_range=3,
                                    pop_ids =pop_ids)
plt.savefig("scSi_"+ind+"_"+sys.argv[1]+"_"+sys.argv[2]+"_"+sys.argv[3]+"_"+sys.argv[4]+"_"+sys.argv[5]+'.pdf')
