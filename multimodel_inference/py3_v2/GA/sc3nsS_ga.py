...
# Fs: factor of Ne reduction (0.001 - 0.99999)
    nu0,nu1,nu2,T1,T2,T3,Fs,P,p_misid = params
...
func=sc12nm
upper_bound = [100,100, 100, 100, 100,100,0.99999,0.999,0.25]
lower_bound = [1e-5,1e-3,1e-3,1e-3,1e-3,1e-3,1e-3,1e-3,1e-5]
params = moments.Misc.perturb_params(params, fold=2, upper_bound=upper_bound,
                              lower_bound=lower_bound)

par_labels = ('nu1','nu2','nu3','T1','T2','T3','F_ne','F_gen','f_misid')
...
moments.ModelPlot.plot_model(plot_mod, save_file="sc3nsS_"+ind+".png", pop_labels=pop_ids, nref=theta/(4*mu), draw_scale=False, gen_time=gtime, gen_time_units="KY", reverse_timeline=True)

# bootstrapping for SDs of params and theta

# printing parameters and their SDs
print( "RESULT","sc3nsS",ind,len(params),ll_model,sys.argv[1],sys.argv[2],sys.argv[3],poptg,theta)
                                    
# plotting quad-panel figure witt AFS, model, residuals:
moments.Plotting.plot_2d_comp_multinom(model, data, vmin=0.1, resid_range=3,
                                    pop_ids =pop_ids)
plt.savefig("sc3nsS_"+ind+"_"+sys.argv[1]+"_"+sys.argv[2]+"_"+sys.argv[3]+"_"+sys.argv[4]+"_"+sys.argv[5]+'.pdf')
