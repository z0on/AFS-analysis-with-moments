...
# Fi: factor of migration reduction (1e-5 - 0.999)
    ...
    fs.integrate([nu1_1*Fs, nu2_1*Fs], T1, m = np.array([[0, 0], [0, 0]]))
    fs.integrate([nu1_2*Fs, nu2_2*Fs], T2, m = np.array([[0, m12], [m21, 0]]))
...
moments.ModelPlot.plot_model(plot_mod, save_file="s103S_"+ind+".png", pop_labels=pop_ids, nref=theta/(4*mu), draw_scale=False, gen_time=gtime, gen_time_units="KY", reverse_timeline=True)

# bootstrapping for SDs of params and theta

# printing parameters and their SDs
print( "RESULT","s103S",ind,len(par_labels),ll_model,sys.argv[1],sys.argv[2],sys.argv[3],poptg,theta)
                                    
# plotting quad-panel figure witt AFS, model, residuals:
moments.Plotting.plot_2d_comp_multinom(model, data, vmin=0.1, resid_range=3,
                                    pop_ids =pop_ids)
plt.savefig("s103S_"+ind+"_"+sys.argv[1]+"_"+sys.argv[2]+"_"+sys.argv[3]+"_"+sys.argv[4]+"_"+sys.argv[5]+'.pdf')
