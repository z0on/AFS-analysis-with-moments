...
# migration is asymmetric, scales with pop size
    ...
    fsi.integrate([nu1_1, nu2_1*Fi], T1, m = np.array([[0, 0], [0, 0]]))
    fsi.integrate([nu1_2, nu2_2*Fi], T2, m = np.array([[0, m12], [m21, 0]]))
...
upper_bound = [100, 100, 100,100,100,100,200,200,0.999,0.99999,0.25]
...
moments.ModelPlot.plot_model(plot_mod, save_file="s02S_"+ind+".png", pop_labels=pop_ids, nref=theta/(4*mu), draw_scale=False, gen_time=gtime, gen_time_units="KY", reverse_timeline=True)

# bootstrapping for SDs of params and theta

# printing parameters and their SDs
print( "RESULT","s02S",ind,len(params),ll_model,sys.argv[1],sys.argv[2],sys.argv[3],poptg,theta)
                                    
# plotting quad-panel figure witt AFS, model, residuals:
moments.Plotting.plot_2d_comp_multinom(model, data, vmin=0.1, resid_range=3,
                                    pop_ids =pop_ids)
plt.savefig("s02S_"+ind+"_"+sys.argv[1]+"_"+sys.argv[2]+"_"+sys.argv[3]+"_"+sys.argv[4]+"_"+sys.argv[5]+'.pdf')
