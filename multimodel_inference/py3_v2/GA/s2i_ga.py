...
# Fi: factor of migration reduction (0.001 - 0.99999)
    nu1_1,nu2_1,T,m12,m21,Fi_12,Fi_21,P,p_misid = params
    ...
    fsi.integrate([nu1_1, nu2_1], T, m = np.array([[0, m12*Fi_12], [m21*Fi_21, 0]]))

    fs2=P*fsi+(1-P)*fs
    return (1-p_misid)*fs2 + p_misid*moments.Numerics.reverse_array(fs2)
 
func=s2mi
upper_bound = [100, 100, 100, 200,200,0.99999,0.999,0.999,0.25]
lower_bound = [1e-5,1e-3,1e-3,1e-5,1e-5,1e-3,1e-3,1e-3,1e-5]
params = moments.Misc.perturb_params(params, fold=2, upper_bound=upper_bound,
                              lower_bound=lower_bound)
par_labels = ('nu1_1','nu2_1','T1','m12','m21','Fi_12','Fi_21','F_gen','f_misid')
...
moments.ModelPlot.plot_model(plot_mod, save_file="s2i_"+ind+".png", pop_labels=pop_ids, nref=theta/(4*mu), draw_scale=False, gen_time=gtime, gen_time_units="KY", reverse_timeline=True)

# bootstrapping for SDs of params and theta

# printing parameters and their SDs
print( "RESULT","s2i",ind,len(params),ll_model,sys.argv[1],sys.argv[2],sys.argv[3],poptg,theta)
                                    
# plotting quad-panel figure witt AFS, model, residuals:
moments.Plotting.plot_2d_comp_multinom(model, data, vmin=0.1, resid_range=3,
                                    pop_ids =pop_ids)
plt.savefig("s2i_"+ind+"_"+sys.argv[1]+"_"+sys.argv[2]+"_"+sys.argv[3]+"_"+sys.argv[4]+"_"+sys.argv[5]+'.pdf')
