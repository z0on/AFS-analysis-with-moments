...
def IMi(params, ns):
    ...
    nu1_funci = lambda t: Fi * nu1_0 * (nu1/nu1_0)**(t/T)
    nu2_funci = lambda t: Fi * nu2_0 * (nu2/nu2_0)**(t/T)
    nu_funci = lambda t: [nu1_funci(t), nu2_funci(t)]

    m21_funci = lambda t: m21 * nu2_funci(t)
    m12_funci = lambda t: m12 * nu1_funci(t)
    migsi = lambda t: np.array([[0, m12_funci(t)], [m21_funci(t), 0]])
    ...
    fsi.integrate(nu_funci, T, dt_fac=0.01, m=migsi)

    fs2=P*fsi+(1-P)*fs
    return (1-p_misid)*fs2 + p_misid*moments.Numerics.reverse_array(fs2)

func=IMi
upper_bound = [100,100,100, 100, 10, 100,100,0.999,0.999,0.25]
lower_bound = [1e-3,1e-3,1e-3,1e-3, 1e-3,1e-5,1e-5,1e-5,1e-5,1e-5]
...
par_labels = ('nu1_0','nu2_0','nu1','nu2','T','m12','m21','F_isl','F_gen','f_misid')
...
moments.ModelPlot.plot_model(plot_mod, save_file="IMi_"+ind+".png", pop_labels=pop_ids, nref=theta/(4*mu), draw_scale=False, gen_time=gtime, gen_time_units="KY", reverse_timeline=True)
