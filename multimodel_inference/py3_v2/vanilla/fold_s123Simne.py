...
# Fs: factor of Ns reduction (1e-5 - 0.999)
    nu0,nu1_1,nu2_1,nu1_2,nu2_2,T0,T1,T2,m12,m21,Fi,Pi,Fs,Ps = params
    ...
    fsi.integrate([nu1_1, nu2_1], T1, m = np.array([[0, m12*nu2_1*Fi], [m21*nu1_1*Fi, 0]]))
    fsi.integrate([nu1_2, nu2_2], T2, m = np.array([[0, m12*nu2_2*Fi], [m21*nu1_2*Fi, 0]]))
    ...
    fsis.integrate([nu1_1*Fs, nu2_1*Fs], T1, m = np.array([[0, m12*nu2_1*Fi*Fs], [m21*nu1_1*Fi*Fs, 0]]))
    fsis.integrate([nu1_2*Fs, nu2_2*Fs], T2, m = np.array([[0, m12*nu2_2*Fi*Fs], [m21*nu1_2*Fi*Fs, 0]]))
    ...
    fss.integrate([nu1_1*Fs, nu2_1*Fs], T1, m = np.array([[0, m12*nu2_1*Fs], [m21*nu1_1*Fs, 0]]))
    fss.integrate([nu1_2*Fs, nu2_2*Fs], T2, m = np.array([[0, m12*nu2_2*Fs], [m21*nu1_2*Fs, 0]]))
...
upper_bound = [100, 100,100, 100,100,100,100,100,200,200,0.999,0.999,0.999,0.999]
lower_bound = [1e-5,1e-5,1e-5,1e-5, 1e-5,1e-5,1e-5,1e-5,1e-5,1e-5,1e-5,1e-5,1e-1,1e-5]
'''
if len(sys.argv)==9:
...
par_labels = ('nu0','nu1_1','nu2_1','nu1_2','nu2_2','T0','T1','T2','m12','m21','Fi','Fgi','Fs','Fgs')
...
moments.ModelPlot.plot_model(plot_mod, save_file="s123Simne_"+ind+".png", pop_labels=pop_ids, nref=theta/(4*mu), draw_scale=False, gen_time=gtime, gen_time_units="KY", reverse_timeline=True)

# bootstrapping for SDs of params and theta

# printing parameters and their SDs
print( "RESULT","s123Simne",ind,len(upper_bound),ll_model,sys.argv[1],sys.argv[2],sys.argv[3],poptg,theta)
                                    
# plotting quad-panel figure witt AFS, model, residuals:
moments.Plotting.plot_2d_comp_multinom(model, data, vmin=0.1, resid_range=3,
                                    pop_ids =pop_ids)
plt.savefig("s123Simne_"+ind+"_"+sys.argv[1]+"_"+sys.argv[2]+"_"+sys.argv[3]+"_"+sys.argv[4]+"_"+sys.argv[5]+'.pdf')
