...
def s10IMS(params, ns):
    ...
    """
    nu0,nu1_0,nu2_0,nu1,nu2,T0,T1,T2,m12,m21,Fs,Ps,p_misid = params
...
func=s10IMS
upper_bound = [100,100,100,100, 100, 10,10,10, 100,100,0.999,0.999,0.25]
lower_bound = [1e-3,1e-3,1e-3,1e-3,1e-3, 1e-3,1e-3,1e-3,1e-5,1e-5,1e-4,1e-4,1e-5]
...
par_labels = ('nu0','nu1_0','nu2_0','nu1','nu2','T0','T1','T2','m12','m21','Fs','F_gs','f_misid')
...
moments.ModelPlot.plot_model(plot_mod, save_file="s10IMSmne_"+ind+".png", pop_labels=pop_ids, nref=theta/(4*mu), draw_scale=False, gen_time=gtime, gen_time_units="KY", reverse_timeline=True)

# bootstrapping for SDs of params and theta

# printing parameters and their SDs
print( "RESULT","s10IMSmne",ind,len(upper_bound),ll_model,sys.argv[1],sys.argv[2],sys.argv[3],poptg,theta)
                                    
# plotting quad-panel figure witt AFS, model, residuals:
moments.Plotting.plot_2d_comp_multinom(model, data, vmin=0.1, resid_range=3,
                                    pop_ids =pop_ids)
plt.savefig("s10IMSmne_"+ind+"_"+sys.argv[1]+"_"+sys.argv[2]+"_"+sys.argv[3]+"_"+sys.argv[4]+"_"+sys.argv[5]+'.pdf')
