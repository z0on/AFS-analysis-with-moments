...
# Fs: factor of Ns reduction (1e-5 - 0.999)
    nu1_0,nu2_0,nu1_1,nu2_1,nu1_2,nu2_2,T0,T1,T2,m12_1,m21_1,m12_2,m21_2,Fi,Pi,Fs,Ps,p_misid = params
    ...
    fsis.integrate([nu1_0*Fs, nu2_0*Fs], T0, m = np.array([[0, 0], [0, 0]]))
    fsis.integrate([nu1_1*Fs, nu2_1*Fs], T1, m = np.array([[0, m12_1*Fi*Fs], [m21_1*Fi*Fs, 0]]))
...
upper_bound = [100, 100,100, 100, 100,100,10,10,10,200,200,200,200,0.999,0.999,0.999,0.999,0.25]
lower_bound = [1e-5,1e-5, 1e-5,1e-5,1e-5, 1e-5,1e-5,1e-5,1e-5,1e-5,1e-5,1e-5,1e-5,1e-5,1e-5,1e-5,1e-5,1e-5]
'''
if len(sys.argv)==9:
...
par_labels = ('nu1_0','nu2_0','nu1_1','nu2_1','nu1_2','nu2_2','T0','T1','T2','m12_1','m21_1','m12_2','m21_2','Fi','Fgi','Fs','Fgs','f_misid')
...
moments.ModelPlot.plot_model(plot_mod, save_file="sc3Si_"+ind+".png", pop_labels=pop_ids, nref=theta/(4*mu), draw_scale=False, gen_time=gtime, gen_time_units="KY", reverse_timeline=True)

# bootstrapping for SDs of params and theta

# printing parameters and their SDs
print( "RESULT","sc3Si",ind,len(upper_bound),ll_model,sys.argv[1],sys.argv[2],sys.argv[3],poptg,theta)
                                    
# plotting quad-panel figure witt AFS, model, residuals:
moments.Plotting.plot_2d_comp_multinom(model, data, vmin=0.1, resid_range=3,
                                    pop_ids =pop_ids)
plt.savefig("sc3Si_"+ind+"_"+sys.argv[1]+"_"+sys.argv[2]+"_"+sys.argv[3]+"_"+sys.argv[4]+"_"+sys.argv[5]+'.pdf')
