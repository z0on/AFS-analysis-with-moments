...
#-------------------
#
def s10IMSi(params, ns):
    """
    ancestral pop size change, then
    split into two non-mixing populations, then 
    migration resumes with exponential growth
    Migration is asymmetric and scales with the size of the source population
    p_misid: proportion of misidentified ancestral states
    Ps: proportion of sites with lower Ne
    Fs: factor of Ns reduction (1e-4 - 0.999)
    Pi: proportion of sites with lower M
    Fi: factor of M reduction (1e-4 - 0.999)
   
    """
    nu0,nu1_0,nu2_0,nu1,nu2,T0,T1,T2,m12,m21,Fs,Ps,Fi,Pi,p_misid = params
    ...
    fsis.integrate(nus_func, T2, dt_fac=0.01, m=migs.si)

    fs2=Pi*(1-Ps)*fsi+Ps*(1-Pi)*fss+Pi*Ps*fsis+(1-Pi)*(1-Ps)*fs
    return (1-p_misid)*fs2 + p_misid*moments.Numerics.reverse_array(fs2)

func=s10IMSi
upper_bound = [100,100,100,100, 100, 10,10,10, 100,100,0.999,0.999,0.999,0.999,0.25]
lower_bound = [1e-3,1e-3,1e-3,1e-3,1e-3, 1e-3,1e-3,1e-3,1e-5,1e-5,1e-4,1e-4,1e-4,1e-4,1e-5]
...
par_labels = ('nu0','nu1_0','nu2_0','nu1','nu2','T0','T1','T2','m12','m21','Fi','F_gi','Fs','F_gs','f_misid')
...
moments.ModelPlot.plot_model(plot_mod, save_file="s10IMSimne_"+ind+".png", pop_labels=pop_ids, nref=theta/(4*mu), draw_scale=False, gen_time=gtime, gen_time_units="KY", reverse_timeline=True)

# bootstrapping for SDs of params and theta

# printing parameters and their SDs
print( "RESULT","s10IMSimne",ind,len(upper_bound),ll_model,sys.argv[1],sys.argv[2],sys.argv[3],poptg,theta)
                                    
# plotting quad-panel figure witt AFS, model, residuals:
moments.Plotting.plot_2d_comp_multinom(model, data, vmin=0.1, resid_range=3,
                                    pop_ids =pop_ids)
plt.savefig("s10IMSimne_"+ind+"_"+sys.argv[1]+"_"+sys.argv[2]+"_"+sys.argv[3]+"_"+sys.argv[4]+"_"+sys.argv[5]+'.pdf')
