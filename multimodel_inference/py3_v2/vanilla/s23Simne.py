...
# Fs: factor of Ne reduction (0.1 - 0.999)
    nu1_1,nu2_1,nu1_2,nu2_2,nu1_3,nu2_3,T1,T2,T3,m12,m21,Fi,Pi,Fs,Ps,p_misid = params
    ...
    fsi.integrate([nu1_1, nu2_1], T1, m = np.array([[0, nu2_1*m12*Fi], [nu1_1*m21*Fi, 0]]))
    fsi.integrate([nu1_2, nu2_2], T2, m = np.array([[0, nu2_2*m12*Fi], [nu1_2*m21*Fi, 0]]))
    fsi.integrate([nu1_3, nu2_3], T3, m = np.array([[0, nu2_3*m12*Fi], [nu1_3*m21*Fi, 0]]))
    ...
    fsis.integrate([nu1_3*Fs, nu2_3*Fs], T3, m = np.array([[0, nu2_3*m12*Fi*Fs], [nu1_3*m21*Fi*Fs, 0]]))
    ...
    fss.integrate([nu1_3*Fs, nu2_3*Fs], T3, m = np.array([[0, nu2_3*m12*Fs], [nu1_3*m21*Fs, 0]]))
...
upper_bound = [100, 100, 100,100,100,100,100,100,10,10,10,0.999,0.999,0.999,0.999,0.25]
lower_bound = [1e-5,1e-5, 1e-5,1e-5,1e-5,1e-5,1e-5,1e-5,1e-5,1e-5,1e-5,1e-3,1e-3,1e-1,1e-3,1e-5]
'''
if len(sys.argv)==9:
...
par_labels = ('nu1_1','nu2_1','nu1_2','nu2_2','nu1_3','nu2_3','T1','T2','T3','m12','m21','Fi','F_gi','Fs','F_gs','f_misid')
...
moments.ModelPlot.plot_model(plot_mod, save_file="s23Simne_"+ind+".png", pop_labels=pop_ids, nref=theta/(4*mu), draw_scale=False, gen_time=gtime, gen_time_units="KY", reverse_timeline=True)

# bootstrapping for SDs of params and theta

# printing parameters and their SDs
print( "RESULT","s23Simne",ind,len(upper_bound),ll_model,sys.argv[1],sys.argv[2],sys.argv[3],poptg,theta)
                                    
# plotting quad-panel figure witt AFS, model, residuals:
moments.Plotting.plot_2d_comp_multinom(model, data, vmin=0.1, resid_range=3,
                                    pop_ids =pop_ids)
plt.savefig("s23Simne_"+ind+"_"+sys.argv[1]+"_"+sys.argv[2]+"_"+sys.argv[3]+"_"+sys.argv[4]+"_"+sys.argv[5]+'.pdf')
