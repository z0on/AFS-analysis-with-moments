...
# Ps: proportion of sites with lower Ne
    nu1,nu2,T,m12,m21,Fi,Pi,Fs,Ps = params
...
func=s2mi
upper_bound = [100, 100, 100, 200,200,0.999,0.999,0.999,0.999]
lower_bound = [1e-5,1e-5,1e-5,1e-5,1e-5,1e-5,1e-5,1e-1,1e-5]
...
par_labels = ('nu1','nu2','T','m12','m21','Fi','F_gi','Fs','F_gs')
...
print( "RESULT","s1Si",ind,len(upper_bound),ll_model,sys.argv[1],sys.argv[2],sys.argv[3],poptg,theta)
                                    
# plotting demographic model
plot_mod = moments.ModelPlot.generate_model(func, poptg, ns)
moments.ModelPlot.plot_model(plot_mod, save_file="s1Si_"+ind+".png", pop_labels=pop_ids, nref=theta/(4*mu), draw_scale=False, gen_time=gtime, gen_time_units="KY", reverse_timeline=True)
...
moments.Plotting.plot_2d_comp_multinom(model, data, vmin=0.1, resid_range=3,
                                    pop_ids =pop_ids)
plt.savefig("s1Si_"+ind+"_"+sys.argv[1]+"_"+sys.argv[2]+"_"+sys.argv[3]+"_"+sys.argv[4]+"_"+sys.argv[5]+'.pdf')
