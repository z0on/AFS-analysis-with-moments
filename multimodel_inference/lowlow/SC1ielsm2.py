...
# split, same size but two migration epochs in each pop, asymmetric migration in first epoch, symmetric in second epoch
# genomic islands
# n(para): 12
...
def sc1ielsm2(params , ns):
#    p_misid: proportion of misidentified ancestral states
    nu1_1, nu2_1, T0, T, m,mi,m12_2, m21_2, m12_2i, m21_2i, P, p_misid = params
...
func=sc1ielsm2
upper_bound = [ 100, 100, 100, 100, 200,200,200,200,200,200,0.999,0.25]
lower_bound = [ 1e-15,1e-15,1e-15,1e-15,1e-5,1e-5,1e-5,1e-5,1e-5,1e-5,0.001,1e-5]
...
moments.ModelPlot.plot_model(plot_mod, save_file="sc1ielsm2_"+ind+".png", pop_labels=pop_ids, nref=theta/(4*mu), draw_scale=False, gen_time=gtime, gen_time_units="KY", reverse_timeline=True)
...
print "RESULT","sc1ielsm2",ind,len(params),ll_model,sys.argv[1],sys.argv[2],sys.argv[3],poptg,theta,uncert
                                    
# plotting quad-panel figure witt AFS, model, residuals:
moments.Plotting.plot_2d_comp_multinom(model, data, vmin=1, resid_range=3,
                                    pop_ids =pop_ids)
plt.savefig("sc1ielsm2_"+ind+"_"+sys.argv[1]+"_"+sys.argv[2]+"_"+sys.argv[3]+"_"+sys.argv[4]+"_"+sys.argv[5]+'.pdf')
