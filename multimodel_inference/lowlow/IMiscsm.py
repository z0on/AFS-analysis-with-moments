...
# split with growth starting after some time, SYMMETRIC migration upon secondary contact, "genomic islands" of two different migration regimes
# n(para): 10 
...
params=array([ 1,1,1,1,1,1,1,1,0.5,0.01])
...
def IM2iSCsm(params, ns):
    ...
    fs.integrate(nu_func, T, dt_fac=0.01, m=np.array([[0, m1], [m1, 0]]))
...
func=IM2iSCsm
upper_bound = [100,100,100, 100, 10,10, 200,200,0.9999,0.25]
lower_bound = [1e-15,1e-15,1e-15,1e-15, 1e-15,1e-15,1e-5,1e-5,1e-4,1e-5]
...
print "RESULT","IMiscsm",ind,len(params),ll_model,sys.argv[1],sys.argv[2],sys.argv[3],poptg,theta,uncert

# plotting quad-panel figure witt AFS, model, residuals:
moments.Plotting.plot_2d_comp_multinom(model, data, vmin=1, resid_range=3,
                                    pop_ids =pop_ids)
plt.savefig("IMiscsm_"+ind+"_"+sys.argv[1]+"_"+sys.argv[2]+"_"+sys.argv[3]+"_"+sys.argv[4]+"_"+sys.argv[5]+'.pdf')

# plotting demographic model
plot_mod = moments.ModelPlot.generate_model(func, poptg, ns)
moments.ModelPlot.plot_model(plot_mod, save_file="IMiscsm_"+ind+"_"+sys.argv[1]+".png",pop_labels=pop_ids, nref=theta/(4*mu), draw_scale=False, gen_time=gtime, gen_time_units="KY", reverse_timeline=True)
