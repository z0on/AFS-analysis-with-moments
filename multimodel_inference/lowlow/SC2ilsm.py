...
# "primary contact"
# split with symmetric migration, two epochs in each pop, symmetric migration only in SECOND epoch
# "genomic islands"
# n(para): 10
...
def sc2ilsm(params , ns):
#    p_misid: proportion of misidentified ancestral states
    nu1_1, nu2_1, nu1_2,nu2_2,T0, T, m,mi, P, p_misid = params
...
func=sc2ilsm
upper_bound = [100, 100, 100, 100, 100, 100, 200,200,0.999,0.25]
lower_bound = [1e-15,1e-15, 1e-15,1e-15,1e-15,1e-15,1e-5,1e-5,1e-15,1e-5]
...
moments.ModelPlot.plot_model(plot_mod, save_file="sc2ilsm_"+ind+".png", pop_labels=pop_ids, nref=theta/(4*mu), draw_scale=False, gen_time=gtime, gen_time_units="KY", reverse_timeline=True)
...
print "RESULT","sc2ilsm",ind,len(params),ll_model,sys.argv[1],sys.argv[2],sys.argv[3],poptg,theta,uncert
                                    
# plotting quad-panel figure witt AFS, model, residuals:
moments.Plotting.plot_2d_comp_multinom(model, data, vmin=1, resid_range=3,
                                    pop_ids =pop_ids)
plt.savefig("sc2ilsm_"+ind+"_"+sys.argv[1]+"_"+sys.argv[2]+"_"+sys.argv[3]+"_"+sys.argv[4]+"_"+sys.argv[5]+'.pdf')
