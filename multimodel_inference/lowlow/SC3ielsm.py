...
# split, three epochs in each pop, symmetric migration in early and late epochs
# genomic islands
# n(para): 13
...
#    p_misid: proportion of misidentified ancestral states
    ...
    fsi.integrate([nu1_1, nu2_1], T1, m = np.array([[0, mi], [mi, 0]]))
...
lower_bound = [1e-15,1e-15, 1e-15,1e-15,1e-15,1e-15,1e-15,1e-15,1e-15,1e-5,1e-5,0.001,1e-5]
...
moments.ModelPlot.plot_model(plot_mod, save_file="sc3ielsm_"+ind+".png", pop_labels=pop_ids, nref=theta/(4*mu), draw_scale=False, gen_time=gtime, gen_time_units="KY", reverse_timeline=True)
...
print "RESULT","sc3ielsm",ind,len(params),ll_model,sys.argv[1],sys.argv[2],sys.argv[3],poptg,theta,uncert
                                    
# plotting quad-panel figure witt AFS, model, residuals:
moments.Plotting.plot_2d_comp_multinom(model, data, vmin=1, resid_range=3,
                                    pop_ids =pop_ids)
plt.savefig("sc3ielsm_"+ind+"_"+sys.argv[1]+"_"+sys.argv[2]+"_"+sys.argv[3]+"_"+sys.argv[4]+"_"+sys.argv[5]+'.pdf')
